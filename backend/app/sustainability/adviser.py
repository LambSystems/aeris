...
def _provider_mode() -> str:
    return os.environ.get("AERIS_LLM_PROVIDER", "gemini").strip().lower()


def _provider_chain():
    mode = _provider_mode()
    if mode == "anthropic":
        return (_try_anthropic_advice,)
    if mode == "auto":
        return (_try_gemini_advice, _try_anthropic_advice)
    return (_try_gemini_advice,)
